...
REGISTERMODEL(m_RC_ZKR_DSO,"Горочная РЦ ЗКР c ДСО")


m_RC_ZKR_DSO::m_RC_ZKR_DSO(QObject *parent):m_RC_Gor(parent)
{
...
m_RC_ZKR_DSO::~m_RC_ZKR_DSO()
{
    for (int i=0;i<_dso2_cnt;i++)
        if (fDSO2[i]) delete fDSO2[i];
}

void m_RC_ZKR_DSO::setDSO(ObjectLink p, int n, int m)
{
    if(flnkdso[n][m]!=p){
        flnkdso[n][m]=p;
        if (p.obj()){
...
void m_RC_ZKR_DSO::setSTATE_DSO_ERR_D(int i, bool p)
{
    // номер с 1!
    if (i==0) qCritical() << "m_RC_ZKR_DSO::setSTATE_DSO_ERR_D(int i, bool p) номер с 1 !" ;
    switch (i) {
...
void m_RC_ZKR_DSO::validation(ListObjStr *l) const
{
    m_RC::validation(l);
    for (int i=0;i<_dso2_cnt;i++){
        for (int j=0;j<2;j++){
...
void m_RC_ZKR_DSO::updateAfterLoad()
{
...
void m_RC_ZKR_DSO::setName(const QString &n)
{
    m_RC::setName(n);
    for (int i=0;i<_dso2_cnt;i++)
        for (int j=0;j<2;j++)
...
bool m_RC_ZKR_DSO::is_33() const
{
    return FSIGNAL_DSO_OSY_COUNT.is33() | FSIGNAL_DSO_STATE.is33();
}

void m_RC_ZKR_DSO::updateStates()
{
    setSTATE_33(is_33());
